...
# [>] GLOBAL VARIABLES
# ---------------------
input_path = Path("programs/2015/Python/09/input.txt")
...
def r_path_min_calculator(
    ...
    for next_city, distance in city_global_info[current_city].items():
        ...
        if next_city in not_visited_city:
            ...
            distance_calculation_try = distance + r_path_min_calculator(next_city, remaining_city_not_visited, city_global_info)
...
def r_path_max_calculator(
    ...
    for next_city, distance in city_global_info[current_city].items():
        ...
        if next_city in not_visited_city:
            ...
            print(f"\t   [!]MATCH: GOING INTO {next_city} exploration! (remaining city: {remaining_city_not_visited})")
            distance_calculation_try = distance + r_path_max_calculator(next_city, remaining_city_not_visited, city_global_info)

            bigger_distance = max(bigger_distance, distance_calculation_try)
...
def path_distance_calculation(city_global_info:dict[str, dict[str, int]]) -> tuple[float, float]:
    ...
    for starting_city in city_global_info:
        ...
        # Recursive Function?
        branch_distance_min = r_path_min_calculator(starting_city, city_to_check, city_global_info )
        branch_distance_max = r_path_max_calculator(starting_city, city_to_check, city_global_info )

        smallest_distance = min(smallest_distance, branch_distance_min)
        longest_distance = max(longest_distance, branch_distance_max)

        print(f" [*] Smallest distance so far? {smallest_distance}")
        


    return smallest_distance, longest_distance
...
def main() -> int:
    ...
    shortest_path_distance, longest_distance = path_distance_calculation(city_global_info)

    print( f"\n\n[>>>] Shortest distance: {shortest_path_distance}km!" )
